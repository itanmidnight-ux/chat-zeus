...
class DecisionEngine:
    ...
    def decide(self, understanding: UnderstandingResult, hot_memory: dict[str, Any] | None = None) -> DecisionPlan:
        ...
        route_scores = {
            'direct': 0.2 + (0.35 if intent in {'simple', 'time', 'date'} else 0.0) + hot_hit_bonus,
        ...
        if hot_memory and intent in {'fact', 'analysis', 'creation'}:
        ...
        plan_steps = [
            ...
            {'step': 'verify', 'status': 'pending', 'detail': 'enabled' if route in {'retrieve', 'analyze', 'create', 'execute'} else 'light'},
        ...
        latency_budget_ms = 500 if route in {'direct', 'math'} else 1600 if route == 'retrieve' else 2200
